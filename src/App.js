...
// ─────────────────────────────────────────────────────────────
// Build constants (v1.0)
// ─────────────────────────────────────────────────────────────
const APP_VERSION = "1.0";
const BUILD_DATE_LABEL = "22/09/2026";
const BRAND = "Studio CAI";
const PRIMARY = "#8B1538";
const LOGO_URL = "https://studio-cai-messenger.vercel.app/logo.jpg";
        ...
        <motion.div layout className="bg-white rounded-3xl shadow-lift overflow-hidden ring-1 ring-neutral-200/80"
          ...
          {fase === "modulo" && (
            <div className="p-6 sm:p-8 space-y-8">
              ...
              <FormSection step={4} icon={<PenLine className="w-4 h-4" />} title="Firma" description="Firma con il dito o con il mouse nel riquadro">
...
const PadFirma = React.forwardRef(function PadFirma({ onChange, iniziale }, ref) {
  ...
  const stato = useRef({ disegno: false, ultimo: null, tratti: 0, firmato: false });
  const [firmato, setFirmato] = useState(false);

  const prepara = () => {
    const cv = canvas.current; if (!cv) return;
    const r = cv.getBoundingClientRect(), d = window.devicePixelRatio || 1;
    const img = stato.current.firmato ? cv.toDataURL("image/png") : null;
    cv.width = r.width * d; cv.height = r.height * d;
    const ctx = cv.getContext("2d");
    ctx.setTransform(d, 0, 0, d, 0, 0);
    ctx.lineCap = "round"; ctx.lineJoin = "round"; ctx.strokeStyle = "#1c1917"; ctx.lineWidth = 2.4;
    if (img) { const im = new Image(); im.onload = () => ctx.drawImage(im, 0, 0, r.width, r.height); im.src = img; }
  };
  useEffect(() => {
    prepara();
    // Tornando al modulo dalla verifica, la firma già fatta viene ridisegnata
    if (iniziale) {
      const cv = canvas.current, r = cv.getBoundingClientRect(), im = new Image();
      im.onload = () => {
        const k = Math.min((r.width - 40) / im.width, (r.height - 40) / im.height, 1);
        const w = im.width * k, h = im.height * k;
        cv.getContext("2d").drawImage(im, (r.width - w) / 2, (r.height - h) / 2, w, h);
      };
      im.src = iniziale;
      stato.current.firmato = true; setFirmato(true);
    }
    window.addEventListener("resize", prepara);
    return () => window.removeEventListener("resize", prepara);
  }, []); // eslint-disable-line

  const pulisci = () => {
    const cv = canvas.current; cv.getContext("2d").clearRect(0, 0, cv.width, cv.height);
    stato.current = { disegno: false, ultimo: null, tratti: 0, firmato: false };
    setFirmato(false); onChange(null);
  };
  React.useImperativeHandle(ref, () => ({ pulisci }));

  const pos = (e) => { const r = canvas.current.getBoundingClientRect(); return { x: e.clientX - r.left, y: e.clientY - r.top }; };
  const giu = (e) => { stato.current.disegno = true; stato.current.ultimo = pos(e); canvas.current.setPointerCapture(e.pointerId); };
  const muovi = (e) => {
  ...
  const su = () => {
    ...
    s.disegno = false;
    if (s.tratti > 15) {
      s.firmato = true; setFirmato(true);
      onChange(esportaFirma(canvas.current));
    }
  };

  return (
    <div>
      <div className={cn("relative rounded-2xl h-44 bg-[#fdfcfa] touch-none", firmato ? "border-[1.5px] border-brand" : "border-[1.5px] border-dashed border-neutral-300")}>
        <canvas ref={canvas} className="w-full h-full block rounded-2xl cursor-crosshair"
          onPointerDown={giu} onPointerMove={muovi} onPointerUp={su} onPointerLeave={su} onPointerCancel={su} />
        <div className="absolute left-6 right-6 bottom-9 border-b border-neutral-200 pointer-events-none" />
        {!firmato && (
      ...
      <div className="flex justify-between items-center mt-2 text-xs">
        <span className={firmato ? "text-green-700 font-semibold" : "text-neutral-500"}>{firmato ? "✓ Firma acquisita" : "Nessuna firma"}</span>
        <button type="button" onClick={pulisci} className="font-bold text-brand inline-flex items-center gap-1"><RotateCcw className="w-3.5 h-3.5" /> Cancella e rifirma</button>
      </div>
    </div>
  );
});
